...
scene.enter(async (ctx) => {
  ...
  saldoRU.saldo(arr[0], arr[1]).then(async (res) => {
    ...
    if (res.nome) {
          ...
          return ctx.replyWithMarkdown(`Você confirma o vínculo ao cartão no nome de *${res.nome}*?`,
      ...
      await ctx.scene.enter("example");
        ...
        scene.action("sim", async (ctx) => {
          ...
          //newData.save();
            ...
            await ctx.replyWithMarkdown("*✅ Cartão vinculado com sucesso!* Digite */saldo* para consultá-lo.");
          });
          

        scene.action("nao", async (ctx) => {
...
bot.command("/saldo", async (ctx) => {
  ...
  if (usersData != '') {
    saldoRU.saldo(usersData[0].CardNumber, usersData[0].Matricula).then((res) => {
      name = res.nome.toLowerCase();
      finalName = name.replace(/(^\w{1})|(\s+\w{1})/g, letter => letter.toUpperCase());
...
*Créditos:* ${res.creditos}`);
    ...
    ctx.replyWithMarkdown(`❌ Parece que você ainda não vinculou nenhum cartão à sua conta. Para vincular, digite */vincular*.`)
  }
});
